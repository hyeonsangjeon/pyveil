...
uv run --isolated --with ./dist/pyveil-0.2.1-py3-none-any.whl python -c "from pyveil import CustomRule, Veil; assert '[PERSON:' in Veil.high(secret=b'test', rules=[CustomRule.exact('PERSON', 'Alice Kim')]).redact_text('Alice Kim').text"
uv run --isolated --with ./dist/pyveil-0.2.1-py3-none-any.whl pyveil demo
uv run --isolated --with ./dist/pyveil-0.2.1-py3-none-any.whl python -m pyveil demo
```
